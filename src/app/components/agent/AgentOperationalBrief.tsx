import { AlertTriangle, BellRing, CalendarClock, CreditCard, FileClock, ListTodo, ReceiptText, Wallet } from 'lucide-react';
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-4 xl:grid-cols-2">
                ...
                <div key={task.id} className="rounded-lg border border-zinc-200 p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-zinc-400">Vence: {task.due_date ? formatDateOnly(task.due_date) : 'Sin fecha'}</p>
                    </div>
          ...
          <BriefListSection
            ...
            emptyText="Sin recordatorios proximos."
          >
              ...
              {data.finances.pendingPayments.map((payment) => (
                <div key={payment.id} className="rounded-lg border border-zinc-200 p-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="text-sm font-semibold text-zinc-900">{formatCurrencyWithSymbol(payment.amount)}</span>
                  </div>
                </div>
              ))}
            </BriefListSection>
        ...
        {data.canSeeFinance ? (
          <div className="grid gap-4 xl:grid-cols-2">
              ...
              {data.finances.overduePayments.map((payment) => (
                <div key={payment.id} className="rounded-lg border border-zinc-200 p-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="text-sm font-semibold text-red-600">{formatCurrencyWithSymbol(payment.amount)}</span>
                  </div>
                </div>
              ))}
            </BriefListSection>
            ...
            <BriefListSection
              title="Cobros recurrentes proximos"
              description="Cobros activos con vencimiento cercano."
              emptyText="Sin cobros proximos."
            >
              {data.finances.upcomingRecurringCharges.map((charge) => (
                <div key={charge.id} className="rounded-lg border border-zinc-200 p-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="text-sm font-semibold text-zinc-900">{formatCurrencyWithSymbol(charge.amount)}</span>
                  </div>
                </div>
              ))}
            </BriefListSection>
              ...
              {data.finances.overdueRecurringCharges.map((charge) => (
                <div key={charge.id} className="rounded-lg border border-zinc-200 p-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="text-sm font-semibold text-red-600">{formatCurrencyWithSymbol(charge.amount)}</span>
                  </div>
                </div>
              ))}
            </BriefListSection>

            {data.actions.recentFinancial.length > 0 ? (
              <BriefListSection
                title="Acciones financieras recientes"
                description="Auditoria breve de acciones sobre pagos y cobros."
                emptyText=""
              >
                {data.actions.recentFinancial.map((action) => (
                  <div key={action.id} className="rounded-lg border border-zinc-200 p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-medium text-zinc-900">{action.action_type}</p>
                        <p className="text-sm text-zinc-500">
