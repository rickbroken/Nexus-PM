...
async function main() {
  ...
  try {
    ...
    const hasBriefAudit = (actionRows ?? []).some(
      (row) =>
        row.action_type === 'nexus_get_daily_brief' &&
        row.input_text === 'mcp:nexus_get_daily_brief'
    );
    const hasPendingTasksAudit = (actionRows ?? []).some(
      (row) =>
        row.action_type === 'nexus_get_pending_tasks' &&
        row.input_text === 'mcp:nexus_get_pending_tasks'
    );
    const hasCreateReminderAudit = (actionRows ?? []).some(
      (row) =>
        row.action_type === 'nexus_create_reminder' &&
