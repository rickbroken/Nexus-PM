...
export function createNexusMcpServer() {
  ...
  for (const tool of mcpToolDefinitions) {
    const runtimeSchema = mcpToolRuntimeSchemas[tool.name as keyof typeof mcpToolRuntimeSchemas];
    const toolConfig = {
      description: tool.description,
      inputSchema: runtimeSchema.shape,
    } as any;

    server.registerTool(tool.name, toolConfig, async (input: Record<string, unknown>) => {
