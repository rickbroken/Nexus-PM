...
function normalizeBriefTask(task) {
    return {
        id: task.id,
        title: task.title,
        status: task.status,
...
function normalizeBriefReminder(reminder) {
    return {
        id: reminder.id,
...
function normalizeRecurringCharge(charge) {
    return {
        id: charge.id,
...
function normalizeFinancialAction(action) {
    return {
        id: action.id,
...
function normalizeFailedAction(action) {
    return {
        id: action.id,
        action_type: action.action_type,
        status: action.status,
...
async function fetchTaskBrief(context) {
...
async function fetchReminderBrief(context) {
    const now = new Date();
...
async function fetchFinanceBrief(context, canSeeFinance) {
    if (!canSeeFinance) {
        return {
...
async function fetchRecentFailedActions(context) {
    const { data, error } = await context.supabaseClient
        .from('agent_actions')
        .select('id, action_type, status, created_at, error_message, entity_type, entity_id, user:users_profiles(id, full_name), project:projects(id, name)')
        .eq('status', 'failed')
        .order('created_at', { ascending: false })
        .limit(MAX_ITEMS);
    if (error)
