...
function extractColumns(openApi, table) {
...
export async function getBackendSchema(context, input, audit) {
    try {
        ...
        if (input.resource === 'buckets' || input.resource === 'all') {
            ...
            result.buckets = (data ?? []).map((bucket) => ({
                id: bucket.id,
                name: bucket.name,
